...
| v2.0 平台化 | 未开始 | 多租户、数据权限和 BPM 尚未进入实现阶段 |
...
| v2.0 | 多租户、数据权限、可选 BPM | 未开始 | 20-35 天起 | 租户隔离 ADR、迁移 POC 和安全评审通过 |

以上周期是单人顺序开发的粗略估算，不包含外部身份平台申请、短信供应商审核、生产运维采购和 BPM 独立选型时间。
...
| 阶段 | 内容 | 退出标准 |
| --- | --- | --- |
| POC | 选择用户、角色、Items 三个模块验证隔离方案 | 跨租户查询和修改均被测试阻断 |
| 基础层 | 租户模型、上下文、依赖、迁移和初始化 | 新请求默认携带明确租户上下文 |
| 系统模块 | 用户、角色、菜单、部门、岗位、字典、参数租户化 | 租户管理员只能管理本租户资源 |
| 业务模块 | 文件、消息、日志和缓存租户化 | 存储路径、缓存 key 和日志均隔离 |
| 运营能力 | 套餐、配额、租户启停和初始化模板 | 禁用租户无法登录 |
...
BPM 不建议从零实现流程引擎。先评估成熟引擎或外部服务，完成请假/审批类最小 POC 后再决定是否纳入主仓库。
...
- 数据权限适用于页面、API 和导出。
- BPM 只有在 POC、运维成本和许可证评审通过后才能进入正式范围。

建议周期：多租户与数据权限 20-35 个工作日；BPM 单独评估，不并入该工期承诺。
...
下一迭代聚焦企业登录、缓存和安全增强，建议任务顺序：

1. 同步 README、PRD、TRD、开发计划和 CHANGELOG 的当前模块状态。
2. 接入 Redis，并明确缓存失效与不可用降级策略。
3. 完成 OAuth2/SSO 协议角色 ADR。
4. 选择一个第三方平台打通真实回调登录。
5. 补齐登录限流、验证码和 MFA 的安全设计。
6. 补齐后端测试、管理员 E2E、普通用户越权 E2E 和 Compose 验证。
