...
```yaml
edition: [base, items, suite]
```

这是当前已交付模块矩阵。IOA、ERP 合入前必须分别增加 `ioa`、`erp` Edition，并把它们加入同一矩阵；`suite` 始终代表所有已交付业务模块的组合。
...
- `pnpm build:edition -- --edition <base|items|suite>` 生成后端只读 Manifest、前端静态注册表和 `artifact-metadata.json`；`--images` 同时构建并标注 OCI 镜像。
